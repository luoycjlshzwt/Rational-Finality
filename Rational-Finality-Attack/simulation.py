...
class LivenessModel(mesa.Model):
    def __init__(self, N=1000000, alpha=0.33, beta=0.01, ell=5, x_min=0.85):
        super().__init__()
        self.num_agents = N
        self.alpha = alpha
        self.beta = beta
...
def run_phase_transition():
    ...
    for b in tqdm(betas):
        model = LivenessModel(N=1000000, alpha=0.33, beta=b, ell=5, x_min=0.85)
...
if __name__ == "__main__":
    ...
    plt.text(0.13, 0.8, "Crash Cost Bound\n$X_{max}-X_{min}=15\%$", color='black', fontsize=22, rotation=0)

    plt.text(0.005, 0.6, "$\sigma^{allH}$", fontsize=19, color='#0072B2', ha='center', weight='bold')
    plt.text(0.08, 0.6, "$\sigma^{A}$", fontsize=19, color='#d35400', ha='center', weight='bold')
    plt.text(0.08, 0.2, "Under-threshold\nCoordination", fontsize=19, color='#d35400', ha='center')
    plt.text(0.175, 0.6, "$\sigma^{allD}$", fontsize=19, color='darkred', ha='center', weight='bold')

    # plt.title(f"Equilibrium Dynamics", fontsize=18, pad=20)
    plt.xlabel(r"Bribe ($\beta$)", fontsize=22, fontweight='bold')
    plt.ylabel(r"Total Deviating Stake ($W$)", fontsize=22, fontweight='bold')
    plt.xlim(0, 0.20)
    plt.ylim(0, 1.05)
    ...
    plt.xlabel(r"Bribe ($\beta$)", fontsize=24, fontweight='bold')
    plt.ylabel(r"Attack Window Length ($\ell$)", fontsize=24, fontweight='bold')
